    ...
    if (error) {
      setError(
        /provider is not enabled/i.test(error.message)
          ? 'Google sign-in is not switched on yet. Please message us on Facebook instead.'
          : error.message
      )
      setBusy(false)
    }
  ...
  return (
    <div>
      <button
        ...
        onClick={start}
        disabled={busy}
        className="flex w-full items-center justify-center gap-3 rounded-md border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-900 disabled:cursor-not-allowed disabled:opacity-60"
      >
        <GoogleG />
